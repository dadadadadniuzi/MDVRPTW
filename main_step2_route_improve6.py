...
from src.aco_improved6_macs import ACO_Improved6_MACS
...
def main():
    output_dir = os.path.join("results", "improved6_aco")
    ...
    print("\n" + "=" * 62)
    print("Start Phase 2: Improve6 MACS-ACO for decomposed VRPTW")
    print("=" * 62)
    ...
    for depot_idx in range(num_depots):
        ...
        best_routes, best_cost, curve = solver.run(
            iterations=max(config.ACO_ITERATIONS, 500),
            ant_count=max(config.ACO_ANT_COUNT, 70),
            seed=42 + depot_idx,
        )
        ...
        filename = f"improved6_route_depot_{depot_idx + 1}.png"
        plot_vehicle_routes(
            depot_coord,
            cluster_data,
            simple_routes_for_plot,
            title=f"Improve6 MACS-ACO - Depot {depot_idx + 1} (Cost: {best_cost:.1f})",
            filename=f"improved6_aco/{filename}",
        )
    ...
    if all_routes_records:
        csv_file = os.path.join(output_dir, "improved6_final_routes.csv")
