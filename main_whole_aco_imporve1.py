...
class WholeNetworkACOImprove1:
    ...
    def run(self):
        ...
        n_ball = max(1, int(n_total * 0.2))
        n_brood = max(1, int(n_total * 0.4))
        n_small = max(1, int(n_total * 0.2))

        for iteration in range(config.ACO_ITERATIONS):
            ant_solutions = []
            ant_costs = []

            for ant_idx in range(n_total):
                ...
                if ant_idx < n_ball:
                    alpha = 2.0
                    beta = 1.0
                elif ant_idx < n_ball + n_brood:
                    alpha = config.ACO_ALPHA
                    beta = config.ACO_BETA
                elif ant_idx < n_ball + n_brood + n_small:
                    alpha = 0.1
                    beta = 4.0
                else:
                    mutation_prob = 0.2

                solution, total_cost = self.construct_solution(alpha, beta, mutation_prob)
                ant_solutions.append(solution)
                ant_costs.append(total_cost)

                if solution is not None and total_cost < best_cost:
                    best_cost = total_cost
                    best_solution = solution

            convergence.append(best_cost)
            self.update_pheromone_improved(ant_solutions, ant_costs, n_ball)
    ...
    def construct_solution(self, alpha, beta, mutation_prob):
        ...
        while unvisited:
            feasible_vehicle_indices = [
                ...
                if self._vehicle_has_feasible_start(vehicle, unvisited, depot_loads)
            ]
            if not feasible_vehicle_indices:
                return None, float("inf")

            vehicle_idx = self._select_vehicle(feasible_vehicle_indices, remaining_vehicles, unvisited, depot_loads)
            vehicle = remaining_vehicles.pop(vehicle_idx)

            route = self._build_route(vehicle, unvisited, depot_loads, alpha, beta, mutation_prob)
            if route is None:
                idle_rounds += 1
            ...
            depot_loads[route["depot_idx"]] += route["load"]
    ...
    def _vehicle_has_feasible_start(self, vehicle, unvisited, depot_loads):
    ...
    def _select_vehicle(self, feasible_vehicle_indices, vehicles, unvisited, depot_loads):
        scored = []
        for idx in feasible_vehicle_indices:
            ...
            feasible_customers = [
                ...
                for c_idx in unvisited
                if self.demands[c_idx] <= vehicle["capacity"]
                and depot_loads[vehicle["depot_idx"]] + self.demands[c_idx] <= self.depot_capacities[vehicle["depot_idx"]]
            ]
            if not feasible_customers:
                continue
            nearest = min(self.depot_customer_dist[vehicle["depot_idx"], c_idx] for c_idx in feasible_customers)
    ...
    def _build_route(self, vehicle, unvisited, depot_loads, alpha, beta, mutation_prob):
        depot_idx = vehicle["depot_idx"]
        depot_tw = self.tw_data["depots"][depot_idx]
        ...
        while True:
            ...
            for c_idx in route_unvisited:
                ...
                if curr_load + demand > vehicle["capacity"]:
                    continue
                if depot_loads[depot_idx] + curr_load + demand > self.depot_capacities[depot_idx]:
                    continue
                ...
                pheromone = self.pheromone[last_index, c_idx + 1] ** alpha
                heuristic = self.heuristic[last_index, c_idx + 1] ** beta
    ...
    def update_pheromone_improved(self, solutions, costs, n_ball):
        self.pheromone *= (1 - config.ACO_RHO)

        ranked = sorted(
            [
                (solution, cost, idx)
                for idx, (solution, cost) in enumerate(zip(solutions, costs))
        ...
        for rank, (solution, cost, ant_idx) in enumerate(ranked[:elite_count]):
...
def plot_depot_routes(output_dir, depots, customers_df, routes):
    for depot_idx in range(len(depots)):
        ...
        plt.title(f"Whole ACO Improve1 Routes - Depot {depot_idx + 1}")
        plt.xlabel("X (km)")
        plt.ylabel("Y (km)")
        plt.grid(True, linestyle="--", alpha=0.4)
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"whole_aco_imporve1_route_depot_{depot_idx + 1}.png"), dpi=300)
        plt.close()


def plot_convergence(output_dir, convergence):
    plt.figure(figsize=(9, 5))
    plt.plot(range(1, len(convergence) + 1), convergence, color="#2ca02c", linewidth=2)
    plt.title("Whole ACO Improve1 Convergence Curve")
    plt.xlabel("Iteration")
    plt.ylabel("Best Cost")
    plt.grid(True, linestyle="--", alpha=0.4)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "whole_aco_imporve1_convergence.png"), dpi=300)
    plt.close()
...
def save_route_results(output_dir, customers_df, routes, best_cost, convergence):
    ...
    df_routes.to_csv(os.path.join(output_dir, "whole_aco_imporve1_routes.csv"), index=False)
    ...
    depot_summary.to_csv(os.path.join(output_dir, "whole_aco_imporve1_depot_summary.csv"), index=False)

    with open(os.path.join(output_dir, "whole_aco_imporve1_summary.txt"), "w", encoding="utf-8") as file:
        file.write("Whole ACO Improve1 Summary\n")
...
def main():
    output_dir = os.path.join("results", "whole_aco_imporve1")
    os.makedirs(output_dir, exist_ok=True)
    np.random.seed(42)

    print(">>> Loading whole-network MDVRPTW data for improved ACO ...")
    ...
    solver = WholeNetworkACOImprove1(customers_df, depots, depot_capacities, vehicle_pool, tw_data)
    best_routes, best_cost, convergence = solver.run()

    if best_routes is None:
        print("No feasible whole-network improved ACO solution was found.")
        sys.exit(1)

    print("\n=== Whole ACO Improve1 Result ===")
