...
from src.aco_improved6_3 import ACO_Improved6_3, tune_improve6_3_params_with_dbo
...
def main():
    output_dir = os.path.join("results", "improved6_3_aco")
    ...
    print("\n" + "=" * 70)
    print("Start Phase 2: Improve6.3 (robust MACS + DBO + anti-stagnation)")
    print("=" * 70)
    ...
    for depot_idx in range(num_depots):
        ...
        best_params, eval_cost = tune_improve6_3_params_with_dbo(problem_args, seed=42 + depot_idx * 101)
        print(f"  DBO tuned eval score: {eval_cost:.2f}")

        solver = ACO_Improved6_3(*problem_args, params=best_params)
        best_routes, best_cost, curve = solver.run(
            iterations=max(config.ACO_ITERATIONS, 620),
            ant_count=max(config.ACO_ANT_COUNT, 84),
            seed=2027 + depot_idx,
        )
        ...
        row = {"Depot_ID": depot_idx + 1, "Tuned_Eval_Score": round(eval_cost, 4)}
        ...
        filename = f"improved6_3_route_depot_{depot_idx + 1}.png"
        plot_vehicle_routes(
            depot_coord,
            cluster_data,
            simple_routes_for_plot,
            title=f"Improve6.3 robust MACS-DBO - Depot {depot_idx + 1} (Cost: {best_cost:.1f})",
            filename=f"improved6_3_aco/{filename}",
        )
    ...
    if all_routes_records:
        routes_csv = os.path.join(output_dir, "improved6_3_final_routes.csv")
        pd.DataFrame(all_routes_records).to_csv(routes_csv, index=False)
        print(f"[Saved] final route file: {routes_csv}")

    if tuned_rows:
        params_csv = os.path.join(output_dir, "improved6_3_tuned_params.csv")
