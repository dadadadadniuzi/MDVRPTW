...
from src.aco_improved3 import ACO_Improved3
...
def main():
    output_dir = os.path.join("results", "improved3_aco")
    ...
    print("\n" + "=" * 60)
    print("Start Phase 2: Improved3 ACO for decomposed VRPTW")
    print("=" * 60)
    ...
    for depot_idx in range(num_depots):
        ...
        filename = f"improved3_route_depot_{depot_idx + 1}.png"
        plot_vehicle_routes(
            depot_coord,
            cluster_data,
            simple_routes_for_plot,
            title=f"Improved3 ACO - Depot {depot_idx + 1} (Cost: {best_cost:.1f})",
            filename=f"improved3_aco/{filename}",
        )
    ...
    if all_routes_records:
        csv_file = os.path.join(output_dir, "improved3_final_routes.csv")
